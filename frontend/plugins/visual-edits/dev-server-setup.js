      ...
      try {
        ...
        for (const change of variableEditChanges) {
          ...
          if (result.success) {
            ...
            try {
              execSync(
                `git -c user.name="visual-edit" -c user.email="noreply@redeagle.local" add "${result.file}"`,
              );
              execSync(
                `git -c user.name="visual-edit" -c user.email="noreply@redeagle.local" commit -m "visual_edit_variable_${timestamp}"`,
              );
            } catch (gitError) {
              console.error(
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(
              `git -c user.name="visual-edit" -c user.email="noreply@redeagle.local" add "${targetFile}"`,
            );
            execSync(
              `git -c user.name="visual-edit" -c user.email="noreply@redeagle.local" commit -m "visual_edit_${timestamp}"`,
            );
          } catch (gitError) {
